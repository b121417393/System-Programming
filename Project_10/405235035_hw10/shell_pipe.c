...
//Save the segmented parameters
char *argVect[256];
char *argVect_2[256];
//Marks whether there is a pipe
int pipe_flag = 0;
...
void ctrC_handler(int sigNumber)
{
    ...
    else
    {
        if (argVect[0] == NULL)
        {
            ...
            fprintf(stderr, "info, User presses ctrl-c while processing the string\n");
    }
}
...
void parseString(char *str, char **cmd, char **cmd_2)
{
    int idx = 0;
    char *retPtr;
    retPtr = strtok(str, " \n");
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        if (strlen(cmdLine) > 1)
            parseString(cmdLine, &exeName, &exeName_2);
        else
            continue;
